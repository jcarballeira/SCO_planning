...
function q = inversekinematic_4dofplanar(robot,  Tf, q0)

fprintf('\nComputing inverse kinematics for the %s robot', robot.name);
fprintf('\nUsing a Moore-penrose pseudo-inverse based method');

%la matriz de posici�n/orientaci�n en Quaternion representation
Qf = T2quaternion(Tf);
Pf = Tf(1:3,4);
...
function qd_null = compute_null_space(robot, q)
J = manipulator_jacobian(robot, q);
I = eye(7);
Jp = penrose(J);
%null space projector: n_space_projector = (I-Jp*J);
